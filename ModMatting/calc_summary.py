import  torch
import numpy as np
def count_params(model, HEIGHT, WIDTH):
    # param_sum = 0
    with open('models.txt', 'w') as fm:
        fm.write(str(model))

    # 计算模型的计算量
    calc_flops(model, HEIGHT, WIDTH)
...
# 计算模型的计算量
def calc_flops(model, HEIGHT, WIDTH):
    ...
    input = torch.rand(1, 3, HEIGHT, WIDTH).to('cuda')
    _,_,_ = model(input, True)
