...
if GPU:
    print('Use GPU...')
    modnet = nn.DataParallel(modnet) #没有这一句会报错 所有key前面会多一个module.
    #modnet = modnet.cuda()
...
while (True):
    ...
    matte_np = max_con.get_max_connect_area(matte_np)
    matte_np = np.repeat(matte_np, 3, axis=2)
    # max_con.get_max_connect_area()
