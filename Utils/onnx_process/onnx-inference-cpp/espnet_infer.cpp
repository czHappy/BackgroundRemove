...
int main()
{
...
#ifdef _WIN32
	const wchar_t* model_path = L"D://ONNX_test/esp_newtrain.onnx";
#else
	const char* model_path = "D://ONNX_test/esp_newtrain.onnx";
#endif
	...
	float mean[3] = {104., 112., 121.};
	for (int c = 0; c < 3; c++) {
		for (int i = 0; i < row; i++) {
			for (int j = 0; j < col; j++) {
				output[c * row * col + i * col + j] = (dst.ptr<uchar>(i)[j * 3 + c] - mean[c]) / 255.0;
			}
        		}
	}
