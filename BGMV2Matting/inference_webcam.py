...
Press B to  between background capture mode and matting mode. The frame shown when B is pressed is used as background for matting.
Press Q to exit.toggle
...
if GPU:
    ...
    model.load_state_dict(torch.load(args.model_checkpoint, map_location=lambda storage, loc: storage), strict=False)
...
def cv2_frame_to_cpu(frame):
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    return ToTensor()(Image.fromarray(frame)).unsqueeze_(0).to('cpu')
...
with torch.no_grad():
    while True:
        ...
        while True: # grab bgr
            ...
            key = dsp.step(frame)
            if key == ord('b'):
                bgr = cv2_frame_to_cpu(cam.read())
                break
            elif key == ord('q'):
                exit()
        while True: # matting
            frame = cam.read()
            src = cv2_frame_to_cpu(frame)
